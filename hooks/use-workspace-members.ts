...
import { workspaceService, userService } from "@/lib/api/services";
            ...
            try {
                ...
                const mappedMembers: WorkspaceMemberWithProfile[] = (membersList as any[]).map((m) => {
                    ...
                    const slugName = userProfile
                        ? [userProfile.firstName, userProfile.lastName].filter(Boolean).join("-").toLowerCase() || userProfile.username || m.userId
                        : m.userId;
